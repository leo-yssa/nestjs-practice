...
@Injectable()
export class MetricInterceptor implements NestInterceptor, OnModuleInit {
  ...
  static registerServiceInfo(serviceInfo: { domain: string; name: string; version: string }): MetricInterceptor {
    new Gauge({
      name: 'nestjs_info',
    ...
    return new MetricInterceptor(new MetricService());
  }
    ...
    return next.handle().pipe(
      tap(() => {
        if (this.isAvailableMetricsUrl(originUrl)) {
          this.metricService.startSuccessTimer(labels);
        }
      }),
      catchError((err) => {
        if (this.isAvailableMetricsUrl(originUrl)) {
          this.metricService.startFailTimer(labels);
          this.metricService.incrementFailureCounter(labels);
        }
        throw err;
      }),
    );
