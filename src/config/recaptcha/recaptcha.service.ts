...
import { RecaptchaFailedException, NotSupportedDeviceException } from './recaptcha.exception';
...
export class RecaptchaService {
  ...
  async verifyRecaptcha(token: string, siteKey: string, ipAddress: string): Promise<void> {
    if (!token || !siteKey) {
    ...
    try {
      ...
      const isSupportedDevice =
        this.options.androidPackageNames.includes(resp.tokenProperties.androidPackageName) ||
      ...
      if (error instanceof RecaptchaFailedException || error instanceof NotSupportedDeviceException) {
        throw error;
      }
      throw new RecaptchaFailedException();
    }
