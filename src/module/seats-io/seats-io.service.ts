...
export class SeatsIOService implements SeatsIOInterface {
  ...
  async holdSeats(eventKey: string, seats: string[]): Promise<void> {
    try {
      const result = await this.client.holdTokens.create(this.options.holdPeriod);
      await this.client.events.hold(eventKey, seats, result.holdToken);
      this.logger.debug(`Successfully held seats ${seats.join(', ')} for event ${eventKey}`);
    } catch (error) {
  ...
  async bookSeats(eventKey: string, seats: string[], holdToken: string): Promise<void> {
    try {
      await this.client.events.book(eventKey, seats, holdToken);
      this.logger.debug(`Successfully confirmed seats ${seats.join(', ')} for event ${eventKey}`);
    } catch (error) {
  ...
  async releaseSeats(eventKey: string, seats: string[]): Promise<void> {
    try {
      ...
      this.logger.debug(`Successfully released seats ${seats.join(', ')} for event ${eventKey}`);
    } catch (error) {
